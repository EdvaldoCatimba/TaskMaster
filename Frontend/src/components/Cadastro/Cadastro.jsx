      ...
      <div className="content-form">
        ...
        <form className="form" onSubmit={handleRegister}>
          ...
          <div className="control">
            ...
            <div className="cont-senha">
              <RiLockPasswordFill />
              <input type="password" nome="confirmarPassword" value={confirmarPassword} onChange={e => setConfirmarPassword(e.target.value)} required />
            </div>
          </div>
          {error && <p className='sms' style={{ color: 'red' }}>{error}</p>}
          {success && <p className='sms' style={{ color: 'green' }}>{success}</p>}
          <button className="btn-entrar" type="submit" disabled={loading}>
            {loading ? 'Cadastrando...' : 'Cadastrar'}
          </button>
        </form>
        <span className='link'>Já tens uma conta? <a href="/login">Entrar</a></span>
      </div>
    </RegisterStyled>
  );
};
